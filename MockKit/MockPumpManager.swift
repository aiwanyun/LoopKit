...
public final class MockPumpManager: TestingPumpManager {
    ...
    public func buildPumpStatusHighlight(for state: MockPumpManagerState) -> PumpStatusHighlight? {
        if state.deliveryIsUncertain {
            ...
            return PumpStatusHighlight(localizedMessage: LocalizedString("没有胰岛素", comment: "Status highlight that a pump is out of insulin."),
                                                         imageName: "exclamationmark.circle.fill",
                                                         state: .critical)
        } else if state.occlusionDetected {
            return PumpStatusHighlight(localizedMessage: LocalizedString("泵阻塞", comment: "Status highlight that an occlusion was detected."),
                                                         imageName: "exclamationmark.circle.fill",
                                                         state: .critical)
        } else if state.pumpErrorDetected {
            return PumpStatusHighlight(localizedMessage: LocalizedString("泵错误", comment: "Status highlight that a pump error occurred."),
                                                         imageName: "exclamationmark.circle.fill",
                                                         state: .critical)
        } else if pumpBatteryChargeRemaining == 0 {
            return PumpStatusHighlight(localizedMessage: LocalizedString("泵电池死亡", comment: "Status highlight that pump has a dead battery."),
                                                         imageName: "exclamationmark.circle.fill",
                                                         state: .critical)
        } else if case .suspended = state.suspendState {
            return PumpStatusHighlight(localizedMessage: LocalizedString("胰岛素暂停", comment: "Status highlight that insulin delivery was suspended."),
